    ...
    <div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header">
                    <h2>
                        Incoming Transactions for the {!!Auth::user()->assigned_to['description']!!}
                    </h2>
                </div>
                <div class="body">
                    @if(count($status)>0)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="form-group">
                                                    {{-- {{ substr_count($s->transaction->description, "\n" )+2 }} --}}
                                                    <textarea rows="4" class="form-control no-resize" readonly>{{$s->transaction->description}}</textarea>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="form-group">
                                                    {{-- {{ substr_count( $s->notes, "\n" )+1 }} --}}
                                                    <textarea rows="4" class="form-control no-resize" readonly>{{$s->notes}}</textarea>
                                                </div>
                                            </td>
                                            <td><a href="/show_transaction/{!!$s->transaction->id!!}">View</a></td>
                                        </tr>
                                    @endforeach 
                                @else
                                    <tr>
                ...
                <div class="footer">
                    <h6>Note: Display transactions from the two months. For transactions beyond two months use "Search Transactions".</h6>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="modal-body">
                    <div class="row clearfix">
                        <form target="_blank" action="/print_transactions" method="GET">
                            ...
                            <div class="col-xs-12">
                                {{--the value 2 means print incoming trasactions to the office--}}
